...
export const PRODUCT_ID = import.meta.env.VITE_PRODUCT_ID || '03d17890-251f-4946-bb91-49d35ff62800'
export const BASE_URL   = import.meta.env.VITE_API_BASE_URL || 'http://localhost:9599/api'

// ─────────────────────────────────────────────────────────────────────────────
// Tipos — Sección 1: Config
// ─────────────────────────────────────────────────────────────────────────────

interface ApiAmount      { value: number; label: string }
interface ApiTerm        { value: number; label: string }
interface ApiInstallment { value: number; label: string }
...
export async function fetchLoanConfig(): Promise<LoanConfig> {
  ...
  if (!res.ok) throw new Error(`fetchLoanConfig: ${res.status} ${res.statusText}`)
